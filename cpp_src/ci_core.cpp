...
get_connections_two_body(const std::vector<SlaterDeterminant>& basis,
                         const std::vector<std::tuple<size_t, size_t, size_t, size_t, Spin, Spin, Spin, Spin, Wavefunction::Coeff>>& terms)
{
    std::vector<SlaterDeterminant> connected_dets;
    std::unordered_set<SlaterDeterminant> unique_dets;

    for (const auto& det : basis) {
        for (const auto& [i, j, k, l, spin_i, spin_j, spin_k, spin_l, v_ijkl] : terms) {
            if (std::abs(v_ijkl) == 0.0) continue;
            auto res = SlaterDeterminant::apply_excitation_double(det, i, j, k, l, spin_i, spin_j, spin_k, spin_l);
            if (res) {
                unique_dets.insert(res->det);
            }
        }
    }

    connected_dets.assign(unique_dets.begin(), unique_dets.end());
    std::sort(connected_dets.begin(), connected_dets.end());
    return connected_dets;
}
