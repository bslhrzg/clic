...
def get_green(clicvars,Ne_dict,h0_0,U_0,thermal_gs,plot_sf = True):
    ...
    if thermal_gs:
        ...
        thermal_state = thermal_gs
    #else:
    #    load_thermal_state() # This method loads from file

    ws = np.array(clicvars.ws)
    iws = np.array(clicvars.iws)

    print(f"DEBUG: here in green_api run : iws is not None : {iws is not None}")
    if iws is not None :
        iws = 1j * np.array(iws)

    target_indices = clicvars.green_block_indices
    ...
    if plot_sf:
        plotting.plot_spectral_function(
            ...
            f"Thermally-Averaged Spectral Function (T={clicvars.temperature}K)",
            "A_w.pdf"
        )
...
def calculate_self_energy(clicvars,
                        ws, 
                        h_imp,
                        G_imp: np.ndarray, 
