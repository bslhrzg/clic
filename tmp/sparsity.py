...
if __name__ == "__main__":
    # one-hot vector (maximally sparse)
    e = np.array([1+0j, 0, 0, 0])
    # uniform vector
    u = np.ones(4, dtype=complex) / 2

    for name, v in [("one-hot", e), ("uniform", u)]:
        print(f"\n{name}:")
        print("  IPR =", ipr(v))
        print("  PR  =", pr(v))
        ...
        print("  N_eps(|c|>=0.5) =", epsilon_support_size(v, 0.5, mode="amplitude"))
        print("  N_eps(p>=0.3)   =", epsilon_support_size(v, 0.3, mode="prob"))